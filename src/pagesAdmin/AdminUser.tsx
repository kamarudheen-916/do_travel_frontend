...
const  AdminUser:React.FC =()=> {
  ...
  useEffect(()=>{
    async function fetchAllUserData (){
      const res = await fetchAllUserDataAPI('user')
      if(res?.data.success){
  ...
  return (
    <div className='bg-slate-800 text-white'>
        ...
        <div className='h-dvh'>
          <TableComponent userType={'user'}  setShowEditModal={setShowEditModal} showEditModal={showEditModal} columns={columns} setRows={setRows} rows={rows}/>
        </div>
    </div>
  )
}
